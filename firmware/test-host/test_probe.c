...
// watchdog reset mid-experiment), and a reply that shipped uninitialised core-0
...
// Leave a recognisable pattern on the stack so an uninitialised read shows up
...
static void test_early_return_is_zeroed(void)
{
    ...
    CHECK(out.quat_w == 0 && out.quat_x == 0 && out.quat_y == 0 && out.quat_z == 0,
          "quaternion is uninitialised stack: %d %d %d %d",
