...
// emits one unified-format UDP datagram per fused sample (stream_type = 3).
//
...
// IMU stream packet (stream_type = 3): the 8-word unified common header, then
// 5 payload words = 10 little-endian int16s. Total 13 words / 52 bytes.
//   w1 TYPE_VER   stream_type=3 | version<<8 | port<<16   (flags bit 0 = port)
