...
| `0xB4` | SET_CONFIG | p1 = config selector | PCAP-swap the whole PL fabric (deferred boot, `docs/deferred-boot.md`): `0`=acquisition, `1`=scan, `2`=acq_imu_both, `3`=acq_imu_port_a, `4`=acq_imu_port_b. Only when not streaming; resets the timestamp on an acquisition load |
